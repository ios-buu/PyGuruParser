...
def print_request_param(username,password,host,database):
    ...
    plt.text(max(x)-max(x)/2, max(y)+100, f"The total of endpoint which has param is {api_count}\nThe quantity of param is {param_count}\nThe average quantity of param in every endpoint is {average}\nThe maximum quantity of parameter's position is {maximum[0]} at {maximum[1]}", ha='left', va='top', fontsize=9)
...
def print_method(username,password,host,database):
    ...
    plt.bar(ind,y,width)
    plt.text(len(x)-6, max(y)+100, f'the maximum quantity of method is {maximum[0]} by {maximum[1]}\nthe minimum quantity of method is {minimum[0]} by {minimum[1]}', ha='left', va='top', fontsize=9)
    plt.show()
    plt.close()
...
def print_api_layer(username,password,host,database):
    ...
    plt.bar(ind,y,width)
    plt.text(max(x)-10, max(y)+100, f'The total of api is {count}\nthe average is {average} layers\nthe maximum quantity of api is in {most_layer}-layer\nthe maximum quantity in layers is {maximum_layer} layers', ha='left', va='top', fontsize=9)
    plt.show()
    plt.close()
